...
openshift:
	...
	sed -i.bak "s|namespace: fivetran-mcp-server|namespace: $(NAMESPACE)|g" deployment/openshift/kustomization.yaml; \
	...
	oc start-build fivetran-mcp-server --from-dir=. --follow || (mv deployment/openshift/deployment.yaml.bak deployment/openshift/deployment.yaml 2>/dev/null; mv deployment/openshift/kustomization.yaml.bak deployment/openshift/kustomization.yaml 2>/dev/null; exit 1); \
	...
	oc get pods -l app=fivetran-mcp-server; \
	echo ""; \
	echo "Useful commands:"; \
	echo "  View logs: oc logs -l app=fivetran-mcp-server --tail=100"; \
	echo "  Get route: oc get route fivetran-mcp-server"; \
	echo "  Check status: oc get pods,svc,route -l app=fivetran-mcp-server"
...
mpp:
	...
	echo "Building container image from source..."; \
	oc start-build fivetran-mcp-server --from-dir=. --follow || (mv deployment/mpp/tenant.yaml.bak deployment/mpp/tenant.yaml 2>/dev/null; exit 1); \
...
undeploy:
	@if [ "$(filter openshift,$(MAKECMDGOALS))" = "openshift" ]; then \
		...
		oc delete deployment,service,route,configmap,secret,pvc,buildconfig,imagestream -l app=fivetran-mcp-server 2>/dev/null || true; \
